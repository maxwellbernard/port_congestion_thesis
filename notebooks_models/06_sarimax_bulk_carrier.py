...
def load_data(path: Path, ports_to_keep: list[str]) -> pl.DataFrame:
    """Load CSV, parse dates, filter to active ports."""
...
print(f"[{DATASET_NAME}] Loaded {df.shape[0]} rows, {len(ports)} ports: {ports}")
...
for target in TARGETS:
    ...
    print(f"\n{'=' * 80}")
    print(f"[{DATASET_NAME}] SARIMAX SUMMARY — {horizon_label}")
    print(f"{'=' * 80}")
...
print("\n" + "=" * 90)
print(f"[{DATASET_NAME}] SARIMAX GRAND SUMMARY — ALL HORIZONS")
print("=" * 90)
